...
fn run_race(race: &Race) -> u64 {
    let min_win = ((race.time as f64 
        - ((race.time.pow(2) - 4 * race.record) as f64).sqrt() )/ 2.0)
        .ceil();
    let max_win = ((race.time as f64 
        + ((race.time.pow(2) - 4 * race.record) as f64).sqrt()) / 2.0)
        .floor();
    // since we're checking for roots, we need to exclude the case where we only tie the record
    if min_win as u64 * (race.time - min_win as u64) == race.record || max_win as u64 * (race.time - max_win as u64) == race.record {
...
fn part1(filename: &str) -> u64 {
    ...
    races.iter().map(|x| run_race(x)).reduce(|acc, e| acc * e).unwrap() as u64
}
